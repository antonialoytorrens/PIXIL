...
#ifdef RCSID
static char *rcsid = "$Id: script.cc,v 1.1 2003/09/08 19:41:59 jasonk Exp $";
#endif
...
void CInputScript::ParseDefineSound()
{
	...
	delay = GetWord();
	mp3 = new Mp3( &m_fileBuf[m_filePos], m_tagEnd-m_filePos, flags );
	mp3->Decompress((short*)buffer, nbSamples);

	delete mp3;
	break;

    default:
	fprintf(stderr, "unknown sound format %lx\n", flags);
	memset(buffer, 0, nbSamples);
	break;
    }
...
void CInputScript::ParseDefineButtonSound()
{
	...
	if (sound) {
		...
		printf("	Sound id %d not a sound (type is %d)\n",
		       (int)soundTag, sound->getType());
	    } else {
